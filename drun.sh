...
docker run --rm -it registry.stackpod.io/workflow/worker:latest -w /examples -r example.traverse.series.1 example.traverse.series.2 example.traverse.series.3 example.traverse.series.4
docker run --rm -it registry.stackpod.io/workflow/worker:latest -w /examples -r example.traverse.parallel.1 example.traverse.parallel.2 example.traverse.parallel.3
