 ...
 *
 * path: string that points to the file / dir
 * opts:
...
export const walkFiles = (path, opts = {}) => {
 ...
 */
export const loadWorkflows = (path, opts = {}) => {

  let wfs = []
  let schema
  const schemaFile = "../lib/workflow_schema.yaml" // TODO - fix tha path
  const ajv2020 = new Ajv2020()

  try {
    schema = yaml.load(fs.readFileSync(schemaFile))
  } catch (err) {
  ...
  if (path == null && opts.workflowYaml)
